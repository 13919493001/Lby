          ...
          <div class="car-item" v-for="(item,index) in carList" :key="index">
            ...
            <div class="car-content">
              ...
              <div class="m-bottom">
                ...
                <div>
                  <img src="@/static/icon/liked.png" alt="" v-if="item.productInfo.collect=='collect'" @click="likeCancel(index)"/>
                  <img src="@/static/icon/like.png" alt="" v-else @click="likeAdd(index)"/>
                  <div @click="changeLike(index)">收藏</div>
                </div>
...
export default {
  ...
  methods:{
      ...
      api.personal.editCar({id:this.carList[index].id,number:this.num}).then((res)=>{
        if(res.status==200){
          ...
          for(let k=0;k<this.checkedCities.length;k++){
            this.payPrice=this.payPrice+parseInt(this.carList[this.checkedCities[k]].truePrice)*this.carList[this.checkedCities[k]].cart_num
          }
        }
      })
    ...
    likeAdd(index){
      this.id=this.carList[index].productInfo.id
      this.carList[index].productInfo.collect="collect"
    ...
    likeCancel(index){
      this.id=this.carList[index].productInfo.id
      this.carList[index].productInfo.collect=""
    ...
    changeLike(index){
      this.id=this.carList[index].productInfo.id
      if(this.carList[index].productInfo.collect=="collect"){
        this.likeCancel(index);
      }else{
        this.likeAdd(index);
      }
    },
    ...
    handleCheckAllChange(val) {
            ...
            if(val==true){
              ...
              for(var i=0; i<this.checkedGameId.length; i++){
                this.price=this.price+parseInt(this.carList[this.checkedGameId[i]].truePrice)*this.carList[this.checkedGameId[i]].cart_num
        ...
        handleCheckedCitiesChange(value) {
          console.log(this.checkedCities);
            ...
            for(var i=0; i<value.length; i++){
              this.price=this.price+parseInt(this.carList[value[i]].truePrice)*this.carList[value[i]].cart_num
        ...
        goOrder(){
          if(this.idList.length>0){
           this.$router.push({name:"carOrder", params: {cartId:this.cartId,id:this.idList,comNum:this.idList.length,numList:this.numList,usual:true,payMoney:this.payPrice}});
