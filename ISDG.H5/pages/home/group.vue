...
export default {
  ...
  methods:{
      purchase(index){
        this.carId=""
        for(var k=0;k<this.groupList[index].product_info.length;k++){
          api.personal.addCar({productId:this.groupList[index].product_info[k].product_id,cartNum:this.groupList[index].product_info[k].number,new:"1",combinationId:this.groupList[index].id}).then((res)=>{
            this.carId=this.carId+res.data.cartId+','
            this.$store.state.carId=this.carId
      ...
      this.$router.push({path:"/home/order", query: {id:this.groupList[index].id}});
    },
  },
  mounted(){
