...
export default {
  ...
  methods: {
    ...
    goodsDatail() {
    this.uniqueId=this.$store.state.uniqueId
    console.log(this.uniqueId,this.$store.state.uniqueId);
      ...
      api.home.goodsDetail(this.id).then(res => {
        ...
        for(var s in res.data.productValue){        
        if(res.data.productValue[s].unique==this.uniqueId){
                this.cate=res.data.productValue[s].suk
        }        
      }
        
        api.personal.addCar({productId:this.id,cartNum:this.carNum,new:"1",uniqueId:this.uniqueId}).then((res)=>{
          ...
          api.home.orderConfirm({cartId:this.cartId}).then((res)=>{
                ...
                this.comList.price=res.data.cartInfo[0].truePrice
                this.price=parseInt(this.comList.price)
                this.payPrice=this.price*this.carNum+this.postage
                this.xPrice=this.price*this.carNum+this.postage
      ...
      if(this.xPrice>=this.minPrice){
        ...
        if(this.integrals>(this.payPrice-this.postage)*100){
            this.integral=Math.ceil(this.payPrice)*100
            this.dprice=this.payPrice    
          }else{
            this.integral=this.integral
    ...
    usingIntegral(val){
      ...
      if(this.checked==true){
        ...
        if(this.integrals>(this.xPrice-this.postage)*100){
            this.integral=Math.ceil(this.xPrice-this.postage)*100
            this.dprice=this.xPrice-this.postage     
          }else{
            this.integral=this.integral
            this.dprice=this.integral/100            
          }
