  ...
  <div class="navbar">
      ...
      <div class="content">
        <div class="content_inner1">
          ...
          <div class="middle">
            ...
            <span v-for="(item,index) in roll" :key="index">{{item.info}}</span>
          </marquee>
          </div>
        </div>
      ...
      <div class="content">
        <div class="left">
          <div class="inner">
            ...
            <a href="https://www.ishokudogen.com/" target="_blank" rel="noopener noreferrer" class="alink">日本官网</a>
          </div>
          <div class="inner">
            <img src="@/static/icon/icon11.png" alt="" />
            <a href="http://qtq.isdg.cn/" target="_blank" rel="noopener noreferrer" class="alink">美妆官网</a>
          </div>
        </div>
        <div class="right">
          <el-popover
          ...
          <div class="inner" v-show="isLogin">
            <img src="@/static/icon/icon2.png" alt="" @click="jump(-1, '/personal')" />
            <div @click="jump(-1, '/personal')">{{nikName}}</div>
            <div @click="exit" class="exit">退出</div>
          </div>
          <div class="inner" @click="goMessage">
            <!-- <el-badge :value="200" :max="99" class="item" v-show="isLogin">
              
            </el-badge> -->
            <el-badge is-dot class="item" v-show="isLogin"><img src="@/static/icon/icon3.png" alt="" /></el-badge>
            <img src="@/static/icon/icon3.png" alt="" v-show="!isLogin" />
            <div class="hidden-xs-only">消息</div>
          </div>
          <div class="inner" @click="jump(-1, '/car')">
          <el-badge :value="cartNum" class="item" v-show="isLogin">
            <img src="@/static/icon/icon4.png" alt="" />
          </el-badge>
            <img v-show="!isLogin" src="@/static/icon/icon4.png" alt="" />
    ...
    <div class="sec2">
      <div class="content">
        ...
        <div class="search">
          <div class="input">
            <input type="search" placeholder="请输入内容" v-model="text" @blur="search" @keyup.enter="search" />
          </div>
...
export default {
  ...
  computed: {
    ...
    watchName(){
      return this.$store.state.nikName
    }
  },
  //监听执行
  watch: {
    ...
    watchName(val) {
    this.nikName=this.$store.state.nikName;
    }
  },
  mounted(){
  ...
  methods: {
    jump(index, path) {
      ...
      if(path=="/car"||path=='/personal'||path=='/login'||path=='/register'){
...
/deep/.el-badge__content.is-fixed{
  ...
  top: 0;
  right: 6px;
  transform: scale(0.5)translateY(-100%)translateX(180%);
}
.alink{
  color: #FFF;
